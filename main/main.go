	...
	for {
		...
		case eventResp := <-eventPoll.Delete():
			go func(eventResp *poll.EventResponse) {
				...
				frm.Document <- donation.Delete(mc.AggCollection, &eventResp.Event)
			}(eventResp)
		...
		case eventResp := <-eventPoll.Insert():
			go func(eventResp *poll.EventResponse) {
				...
				frm.Document <- donation.Insert(mc.AggCollection, &eventResp.Event)
			}(eventResp)
		...
		case eventResp := <-eventPoll.Update():
			go func(eventResp *poll.EventResponse) {
				...
				frm.Document <- donation.Update(mc.AggCollection, &eventResp.Event)
			}(eventResp)
		}
	}
}
